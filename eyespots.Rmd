...
```
**Figure 1:** Probability of predation higher for blank design. Plot shows mean probabilities of predation across three designs of artificial Lepidoptera left exposed for 48 hours. Design 1 (blank), design 2(1 eyespot) and design 3 (2 eyespots). Y axis represents predicted probabilities of predation, calculated using the emmeans function which analysed a binomial logit-link generalized linear model (Table 1). Solid points represent mean probabilities. Horizontal lines and labels represent upper and lower 95% confidence intervals. X axis represent the three designs. 
...
A descriptive summary of the raw data indicated there may be a difference in predation levels of models with eyespots containing a 'sparkle' compared to other designs at location 1, but in location 2, predation levels were almost equal for all designs. Due to this observation, an interaction term between location and design was added to the model. The indication of a potential interaction was supported by the predicted probabilities of predation in each location (figure 2).
...
```
**Figure 3:** Models with 'sparkles' within eyespots show slightly lower probability of predation, but this difference is not significant. Design 1 ('sparkle' within eyespots), design 2 ('sparkle' outside of eyespots), design 3 (control). Plot shows mean probabilities of predation across three designs of artificial Lepidoptera left exposed for 24 hours. Values obtained using emmeans function, which analysed a binomial logit-link generalized linear model (Table 3). Y axis represents predicted probabilities of predation with solid points representing mean probabilities. Horizontal lines and labels represent upper and lower 95% confidence intervals. X axis represent the three designs. 
...
```
**Figure 4:** The impact of learning on predation. The plot shows the relationship between probability of predation and time into the experiment at each location (left = location 1, right = location 2). Y axis represents probability of predation and x axis represents collection event number. Blue represents design 1 (blank), red represents design 2 (1 eyespot) and green represents design 3 (2 eyespots). Artificial Lepidopteran models were exposed to predators in 48 hour periods across 9 collection events. At location 2, predation levels were significantly influenced by collection (or time into the experiment). This indicates that learning significantly influenced the probability of predation at location 2 but not location 1. Despite this, there is an increase in probability of predation after collection 2 at location 1. The shaded zones represent the 95% confidence intervals of predictions for each design. Predictions were made using the augment_glm function which analysed a binomial logit-link generalized linear model.
